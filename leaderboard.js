...
async function updateLeaderBoard(newEntry) {
  const localURL = 'http://localhost:5000/api/v1/leaderboard';
  const response = await fetch(localURL, {
    method: newEntry ? 'POST' : 'GET',

    headers: { 'Content-Type': 'application/json' },
    body: newEntry ? JSON.stringify(newEntry) : undefined,
  });
  const json = await response.json();
  const tempNewLeaderboard = json
    .map(({ name, score, date }) => ({
      name,
      score,
      date: new Date(date),
    ...
    .slice(0, 5);
  if (tempNewLeaderboard !== cachedLeaderboard) {
    cachedLeaderboard = leaderboard;
    leaderboard = tempNewLeaderboard;
  }
}
